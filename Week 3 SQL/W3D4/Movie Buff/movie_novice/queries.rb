...
def below_average_years
  ...
  Movie.select('COUNT(title) as bad_movies', 'yr').where('score < ?', 5).group('yr').order('COUNT(title)')
end
...
def pulp_fiction_actors
  ...
  Actor.select('id','name').joins(:movies).where('title like ?', 'Pulp%')
end
...
def uma_movies
  ...
  Movie.select('id','title','yr').joins(:actors).where('name like ?', 'Uma%')
end
